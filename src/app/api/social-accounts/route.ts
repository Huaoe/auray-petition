...
import { getUserIdServer, getOrCreateUserIdServer } from "@/lib/user-session";

export async function GET(request: NextRequest) {
  try {
    // Get user ID from session
    console.log("Using getOrCreateUserIdServer in social-accounts route");
    const sessionUserId = await getOrCreateUserIdServer();
    ...
    const accounts: SocialMediaAccount[] = credentials.map((cred) => ({
      ...
      isExpired: cred.tokenExpiry
        ? new Date(cred.tokenExpiry) < new Date()
        : false,
    }));
