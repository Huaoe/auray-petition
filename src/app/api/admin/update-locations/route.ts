...
export async function POST(request: NextRequest) {
  ...
  // This is a simplified example without auth checks
  ...
  try {
    ...
    if (!process.env.UNSPLASH_ACCESS_KEY) {
      return NextResponse.json(
        { success: false, error: 'UNSPLASH_ACCESS_KEY environment variable is not set' },
        { status: 500 }
      );
    }
    
    // Create a deep copy of the locations to modify
    const updatedLocations: Record<string, FamousLocation[]> = JSON.parse(JSON.stringify(FAMOUS_LOCATIONS));
    ...
    for (const [category, locations] of Object.entries(updatedLocations)) {
      console.log(`Processing category: ${category} (${locations.length} locations)`);
      ...
      for (let i = 0; i < locations.length; i++) {
        ...
        try {
          console.log(`[${globalIndex + 1}/${totalLocations}] Updating ${location.name}...`);
